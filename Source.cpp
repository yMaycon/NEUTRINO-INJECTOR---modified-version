...
int main()
{
	...
	CEasyRegistry* reg = new CEasyRegistry(HKEY_LOCAL_MACHINE, "Software\\WOW6432Node\\Multi Theft Auto: San Andreas All\\Common", false);
	if (reg)
	{
		...
		if (rslt)
		{
			...
			if (rslt)
			{
				...
				PVOID oldCodeMem = VirtualAllocEx(processInfo.hProcess, NULL, 4059, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
				if (!oldCodeMem)
				{
